...
#ifndef R3BPSPXPOSCALPAR_H
#define R3BPSPXPOSCALPAR_H
...
class R3BPspxPoscalPar : public FairParGenericSet
{

  public:
    /** Standard constructor **/
    R3BPspxPoscalPar(const char* name = "PspxPoscalPar",
                  const char* title = "Pspx POSCAL parameters",
                  const char* context = "Default");

    // Destructor
    virtual ~R3BPspxPoscalPar();
  ...
  private:
    ...
    R3BPspxPoscalPar(const R3BPspxPoscalPar&); //a copy constructor
    const R3BPspxPoscalPar& operator=(const R3BPspxPoscalPar&); // an  assignment operator

    ClassDef(R3BPspxPoscalPar, 1);
};
