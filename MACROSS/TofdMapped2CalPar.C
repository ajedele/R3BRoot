...
void TofdMapped2CalPar(int RunId);

void TofdMapped2CalPar(){
	TofdMapped2CalPar(220); // 226 is last full run
}
...
void TofdMapped2CalPar(int RunId)
{
	...
	TString outputFileName = path2 + "TofdTCalPar" + runNumber1 + "_Test.root";
	...
	R3BTofDMapped2CalPar* tofdMapped2CalPar = new R3BTofDMapped2CalPar("TofdTCalPar", 1);
	tofdMapped2CalPar->SetNofModules(4,44);
	run->AddTask(tofdMapped2CalPar);
